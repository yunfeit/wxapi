...
2. 使用 `composer`
  ...
  ```shell
  #安装composer依赖
  composer require "Yunfeit/wechat-php-sdk:1.*"
...
3. `ThinkPHP` 内使用
  ...
  将SDK内 `src` 文件夹重命名为 `Yunfeit`, 拷贝至 `ThinkPHP/Library/` 下即可使用 `Wechat` 和 `Api` 类库.
  ...
  ```php
  $wechat = new \Yunfeit\WechatPhpSdk\Wechat(array(		
  	'appId' => $appId,	
  	'token' => 	$token,
  	'encodingAESKey' =>	$encodingAESKey
  ));

  $api = new \Yunfeit\WechatPhpSdk\Api(
    array(
        'appId' => $appId,
        'appSecret'	=> $appSecret,
